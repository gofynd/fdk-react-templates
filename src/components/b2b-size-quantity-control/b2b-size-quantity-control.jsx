...
function B2BSizeQuantityControl({
  ...
  const handleDecrement = (e) => {
    ...
    clearInternalError();
    setQuantity((prev) => {
      const newQty = prev - incrementDecrementUnit;
      return newQty < minCartQuantity ? minCartQuantity : newQty;
    });
    onDecrementClick(e);
  };
  ...
  const handleIncrement = (e) => {
    ...
    setQuantity((prev) => {
      const newQty = prev + incrementDecrementUnit;
      return newQty > maxCartQuantity ? maxCartQuantity : newQty;
    });
    onIncrementClick(e);
  };
  ...
  const onCurrentQtyChange = (evt) => {
    ...
    if (newQty && newQty > 0) {
      if (newQty % incrementDecrementUnit !== 0) {
        ...
        setInternalError(`Max Order Quantity is ${maxCartQuantity}`);
      }
    }
  };
