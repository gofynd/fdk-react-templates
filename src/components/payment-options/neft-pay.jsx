...
function NeftPay({
  ...
  const handlePlaceOrder = () => {
    ...
    if (isUploadFieldRequired) {
      if (!neftFileUpload?.state?.fileUploaded || neftFileUpload?.state?.uploadedFileUrl?.length === 0) {
  ...
  const handleFileRemove = (fileIndex) => {
    ...
    if (neftFileUpload?.reset) neftFileUpload.reset(fileIndex);
  };
  ...
  const handleFileInputChange = async (event) => {
    ...
    if (neftFileUpload?.upload) {
      for (const file of files) await neftFileUpload.upload(file);
    }
  };
      ...
      <div className={styles.neftWrapper}>
        ...
        <div className={styles.neftFormBlock}>
          ...
          <section className={`${styles.neftSection} ${styles.neftUploadSection}`}>
            ...
            {(neftFileUpload?.state?.fileUploadError || fileUploadError) && (
            ...
            {proofFiles?.some((_, index) => index < neftFileUpload?.state?.fileUploadedName?.length) && (
              <div className={styles.neftUploadedFilesWrapper}>
                {proofFiles.map((file, index) => {
                  if (index >= neftFileUpload?.state?.fileUploadedName?.length) return null;
                  return (
