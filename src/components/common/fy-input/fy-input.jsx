 ...
 * @param {string} inputVariant - The style variant of the input field. Common options could be "outlined", "filled", "standard".
...
const FyInput = forwardRef(
  (
    ...
    const customInputClassName = useMemo(
      () => `${styles.fyInput} ${styles[inputVariant]} ${inputClassName ?? ""}`,
      [inputVariant, inputClassName]
    );
      ...
      <div className={customContainerClassName}>
        {label && (
          <label htmlFor={id} className={customLabelClassName}>
            {label}
            {props?.required && showAsterik && (
              <span className={styles.required}> *</span>
            )}
          </label>
        )}
        <input {...props} id={id} className={customInputClassName} ref={ref} />
