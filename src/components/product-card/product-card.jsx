...
const ProductCard = ({
  ...
  // Intersection observer to load images when in viewport
  const loadImage = (src, setSrc) => {
  ...
  useEffect(() => {
    const imgSrc = getProductImages()?.[0]?.url || "";

    const observer = loadImage(imgSrc, setImageSrc);
