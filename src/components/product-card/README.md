...
| `showImageOnHover`   | `bool`         | `false`         | Deprecated legacy prop. Maps to `imageEffects="swap_image"` when `imageEffects` is not provided. |
| `showMultipleImages` | `bool`         | `false`         | Deprecated legacy prop. Maps to `imageEffects="show_multiple_images"` when `imageEffects` is not provided. |
