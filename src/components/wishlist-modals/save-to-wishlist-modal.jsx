...
function SaveToWishlistModal({
  ...
  const wishlistObject = (wishlists) => {
    const response = wishlists.map((wishlist) => ({
      ...
      createdAt: formatDate(wishlist.created_at),
          ...
          <div className={styles.wishlistModalContent}>
            {isLoading ? (
              <div className={styles.loadingState}>
                ...
                <p>{t("resource.dynamic_label.modal_loading_wishlists")}</p>
              </div>
            ) : (
              <div className={styles.wishlistList}>
                {wishlists.length === 0 ? (
                  <div className={styles.emptyState}>
                    <p>
                      {t("resource.dynamic_label.modal_no_wishlists_found")}
                    </p>
                  </div>
                ) : (
                  <>
                      ...
                      >
                        ...
                        <div className={styles.wishlistInfo}>
                          ...
                          <div className={styles.wishlistMeta}>
                            {wishlist.itemCount}{" "}
                            {t("resource.dynamic_label.items")} •{" "}
                            {t("resource.dynamic_label.modal_created")}:{" "}
                            {wishlist.createdAt}
                          </div>
                        </div>
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      {isLastVisible && (
                                        ...
                                        >
                                          <div>
                                            +{wishlist.previewImages.length - 1}
                                          </div>
                                          <div>{t("resource.dynamic_label.modal_more")}</div>
                                        </div>
                                      )}
                                    </div>
                                  );
          ...
          <div className={styles.wishlistModalFooter}>
            <div className={`${styles.mobileOptionMenu} mobile-option-menu`}>
              <OptionMenu
                options={[
                  {
                    label: t("resource.dynamic_label.modal_create_new_wishlist"),
                    onClick: () => {
                      onClose();
                      handleOpenCreateWishlistModal();
                    },
