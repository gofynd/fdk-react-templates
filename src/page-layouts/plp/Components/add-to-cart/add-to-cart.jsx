...
const APPAREL_SIZE_LABEL_PATTERN =
  /^(OS|ONE SIZE|ONESIZE|XXS|XS|S|M|L|XL|XXL|XXXL)$/i;

const parseCosCompositeSize = (value) => {
  const match = String(value || "").match(/^\s*(\d{2})\s*\/\s*(\d{3})\s*$/);
  if (!match) return null;
  return { sizeCode: match[1], colorCode: match[2] };
};

const getCosNumericSizeLabel = (value) =>
  COS_NUMERIC_SIZE_LABELS[String(value || "").trim()] || "";

const getSizeDisplayValue = (size = {}) => {
  if (typeof size === "string") return getCosNumericSizeLabel(size) || size;
  ...
  const directLabel =
    ...
    size?.size ||
    size?.size_label;
  if (directLabel) return getCosNumericSizeLabel(directLabel) || directLabel;

  const parsedSize =
    parseCosCompositeSize(size?.value) || parseCosCompositeSize(size?.display);
  if (parsedSize?.sizeCode) {
    return COS_SIZE_CODE_LABELS[parsedSize.sizeCode] || parsedSize.sizeCode;
  }

  return (
    getCosNumericSizeLabel(size?.display) ||
    getCosNumericSizeLabel(size?.value) ||
    size?.display ||
    size?.value ||
    ""
  );
...
const getSizeChartDisplayList = (sizes = {}) => {
  const sizeChartRows = sizes?.size_chart?.sizes;
  if (!Array.isArray(sizeChartRows)) return [];

  return sizeChartRows
    .map((row) => ({
      display:
        row?.col_1 ||
        row?.size ||
    ...
    .filter((size) => size.display);
};

const isBareNumericSizeLabel = (value) =>
  /^\d+$/.test(String(value || "").trim());

const hasOnlyBareNumericSizeLabels = (sizes = []) =>
  sizes.length > 0 &&
  sizes.every((size) => isBareNumericSizeLabel(size?.display ?? size?.value));

const hasApparelSizeLabels = (sizes = []) =>
  sizes.some((size) =>
    APPAREL_SIZE_LABEL_PATTERN.test(
      String(size?.display ?? size?.value ?? "").trim()
    )
  );

const resolveDisplaySizeOptions = (sizeOptions = [], sizes = {}) => {
  const normalizedSizes = toArray(sizeOptions).map((size) =>
    typeof size === "string"
      ? { display: getSizeDisplayValue(size), value: size }
      : {
          ...size,
          display: getSizeDisplayValue(size),
        }
  );
  const chartDisplayList = getSizeChartDisplayList(sizes);

  if (
    hasOnlyBareNumericSizeLabels(normalizedSizes) &&
    hasApparelSizeLabels(chartDisplayList)
  ) {
    return normalizedSizes.map((size, index) => ({
      ...size,
      display: chartDisplayList[index]?.display || size?.display,
    }));
  }

  if (
    normalizedSizes.length === 1 &&
    sizes?.multi_size === false &&
    isBareNumericSizeLabel(
      normalizedSizes[0]?.display ?? normalizedSizes[0]?.value
    )
  ) {
    return [{ ...normalizedSizes[0], display: "OS" }];
  }
...
const AddToCart = ({
  ...
  const safeGlobalConfig = globalConfig || {};
  ...
  const { product = {}, productPrice = {} } = safeProductData;
  ...
  const safePricePerPiece = productPrice?.price_per_piece || {};
  ...
  const sizeOptions = useMemo(
    () => resolveDisplaySizeOptions(sizes?.sizes, sizes),
    [sizes]
  );
  const quickShopFulfillmentOptions = useMemo(
    () => toArray(fulfillmentOptions),
    [fulfillmentOptions]
  );
  ...
  const isSizeSelectionBlock =
    isQuickShop || safePageConfig?.size_selection_style === "block";
  const isSingleSize = displaySizeOptions?.length === 1;
  const isSizeCollapsed = safePageConfig?.hide_single_size && isSingleSize;
  const preSelectFirstOfMany = safePageConfig?.preselect_size;
  ...
  const getProductPrice = (key) => {
    ...
    if (selectedSize && !isEmptyOrNull(productPrice?.price)) {
      if (productPrice?.set && !isEmptyOrNull(safePricePerPiece)) {
        return (
          currencyFormat(
            safePricePerPiece?.[key],
            safePricePerPiece?.currency_symbol ||
  ...
  const quickShopRecommendations = useMemo(() => {
    return toArray(recommendationProducts)
      .filter((item) => item?.uid !== product?.uid && item?.slug !== product?.slug)
      .slice(0, 4);
  }, [recommendationProducts, product?.uid, product?.slug]);
  const variantOptions = useMemo(() => toArray(variants), [variants]);
  const colorVariant = useMemo(() => {
    return variantOptions.find((variant) =>
      ["color", "image"].includes(variant?.display_type)
    );
  }, [variantOptions]);
  const colorVariantItems = useMemo(
    () => toArray(colorVariant?.items),
    [colorVariant?.items]
  );
  const selectedColorVariant = useMemo(() => {
    const selectedVariant = colorVariantItems.find((variant) =>
      isSelectedVariant(variant, slug, product?.slug)
    );
    return selectedVariant || colorVariantItems[0] || {};
  }, [colorVariantItems, slug, product?.slug]);
  const quickShopGalleryImages = useMemo(() => {
    ...
    const selectedVariantImages = Array.isArray(selectedColorVariant?.medias)
      ...
      : [];
    const allVariantImages = colorVariantItems.flatMap((variant) =>
  ...
  }, [media, selectedColorVariant, colorVariantItems]);
  ...
  const selectedSizeDisplay = selectedSizeItem
    ? getSizeDisplayValue(selectedSizeItem)
    : selectedSize;
  const shouldShowEffectiveSelectedSize =
    Boolean(selectedSize) || isProductLevelOutOfStock;
  const effectiveSelectedSize = shouldShowEffectiveSelectedSize
    ? selectedSizeDisplay || ""
    : "";
              ...
              >
                <div className={styles.wishlistToastImage}>
                  <FyImage
                    ...
                    sources={[{ width: 140 }]}
                    globalConfig={safeGlobalConfig}
                    isImageFill={safeGlobalConfig?.img_fill}
                    backgroundColor={
                      safeGlobalConfig?.img_container_bg ||
                      "oklch(96% 0.006 100)"
                    }
                  />
                </div>
            ...
            <div className={styles.productScrollContent}>
              ...
              {isSizeSelectionBlock && !isSizeCollapsed && (
                <div className={styles.sizeSelection}>
                  <div className={styles.sizeHeaderContainer}>
                    ...
                    {(isQuickShop || safePageConfig?.show_size_guide) &&
                  ...
                  <div className={styles.sizeSelection__wrapper}>
                    {displaySizeOptions?.map((size, index) => {
                      const sizeDisplay = getSizeDisplayValue(size);
                      ...
                      return (
                        <button
                          type="button"
                          key={`${size?.value || sizeDisplay}-${index}`}
              ...
              {sizes?.sellable && selectedSize && !isOutOfStockQuickShop && (
                <DeliveryInfo
                  ...
                  mandatoryPincode={safePageConfig?.mandatory_pincode}
                />
              )}

              {selectedSize &&
                !isOutOfStockQuickShop &&
                !!quickShopFulfillmentOptions.length &&
                availableFOCount > 1 && (
                  <div className={styles.fulfillmentWrapper}>
                    <div className={styles.foList}>
                      {foLoading
                        ? quickShopFulfillmentOptions.map((_, index) => (
                            <div
                        ...
                        : quickShopFulfillmentOptions.map((foItem, index) => (
                            <FullfillmentOption
                              key={index}
                              foItem={foItem}
                              fulfillmentOptions={quickShopFulfillmentOptions}
                              currentFO={currentFO}
...
const getRecommendationImage = (product) => {
  const media = toArray(product?.media).length
    ? toArray(product?.media)
    : toArray(product?.medias);
...
const getVariantImage = (variant = {}) => {
  const medias = toArray(variant?.medias);
...
const getSizeStockMessage = (size = {}, isMto = false, isSelectedLabel = false) => {
  ...
  if (isMto || !Number.isFinite(quantity) || quantity <= 0) return "";
  if (quantity === 1) return isSelectedLabel ? "Last one left" : "One left";
  if (quantity <= 5) return "Few in stock";
...
const QuickShopNotifyPanel = ({
  ...
  const safeGlobalConfig = globalConfig || {};
  const trendingItems = toArray(recommendations).slice(0, 2);
...
const getStoreAvailabilityRows = (sizes = [], selectedSize = "") => {
  ...
  if (!sizeList.length) return FALLBACK_STORE_SIZES;
...
const FullfillmentOption = ({
  ...
  const formattedPromise =
    typeof getDeliveryPromise === "function"
      ? getDeliveryPromise(foItem?.delivery_promise)
      : "";
  const fulfillmentOptionList = toArray(fulfillmentOptions);
  return (
    <div
      className={styles.fulfillmentOption}
      onClick={() => setCurrentFO(foItem?.fulfillment_option || {})}
    >
      {fulfillmentOptionList.length === 1 ? (
