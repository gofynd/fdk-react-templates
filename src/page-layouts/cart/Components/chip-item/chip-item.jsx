...
export default function ChipItem({
  ...
  const cartUpdateHandler = (
    ...
    if (!itemDetails?.custom_order?.is_custom_order) {
      if (
        totalQuantity - incrementDecrementUnit < maxCartQuantity &&
        totalQuantity > maxCartQuantity
      ) {
        totalQuantity = maxCartQuantity;
      } else if (totalQuantity > maxCartQuantity) {
        setShowQuantityError(true);
        return;
      }
      if (
        totalQuantity + incrementDecrementUnit > minCartQuantity &&
        totalQuantity < minCartQuantity
      ) {
        totalQuantity = minCartQuantity;
      } else if (totalQuantity < minCartQuantity) {
        totalQuantity = 0;
      }
    }
