...
const JuspayCardForm = ({paymentResponse, paymentOption, getCurrencySymbol, getTotalValue}) => {
  ...
  useEffect(() => {
    const loadJuspayScript = async () => {
      try {
        await injectScript(paymentOption?.payment_flows?.juspay?.data?.sdk_link);
        setTimeout(() => {
          if (window.Juspay) {
  ...
  const getInvalidFieldError = (fieldName, event) => {
    ...
    if ((fieldName === 'card_exp_month' || fieldName === 'card_exp_year') && event.expiry_valid === false) {
  ...
  const isCardValid = () => {
    ...
    const allFieldsCompleted = Object.values(fieldCompleted).every(completed => completed);
    ...
    <>
      {!isJuspayLoaded ? (
        ...
        <form className="juspay_inline_form" id="payment_form">
          <input type="hidden" className="merchant_id" value={paymentResponse?.data?.merchant_id} />
          <input type="hidden" className="order_id" value={paymentResponse?.data?.order_id} />
          
          <div className={`${styles.cardInputWrapper} ${styles.cardNumberBox}`}>
              <div className={`card_number_div`}></div>
              {fieldErrors.card_number && (
                <div className={`${styles.formError}`}>{fieldErrors.card_number}</div>
              )}
          </div>
          ...
          <div className={styles.cardInputWrapper}>
            ...
            {fieldErrors.name_on_card && (
              <div className={`${styles.formError}`}>{fieldErrors.name_on_card}</div>
            )}
          </div>
          
          <div className={`${styles.cardDateCvvJuspayWrapper} ${styles.cardInputWrapper}`}>
              <div className="card_exp_month_div"></div>
              <div className="card_exp_year_div"></div>
              <div className="security_code_div"></div>
          </div>
          {(fieldErrors.card_exp_month || fieldErrors.card_exp_year || fieldErrors.security_code) && (
            <div className={`${styles.formError}`}>
              {fieldErrors.card_exp_month || fieldErrors.card_exp_year || fieldErrors.security_code}
            </div>
          )}
          
          <div className={`${styles.checkboxWrapper} ${styles.cardInputWrapper}`}>
            <input type="checkbox" className={`${styles.checkbox}`} id="save_card" />
            <label htmlFor="save_card">{t("resource.payment.save_card_information")}</label>
          </div>
