...
function CheckoutPaymentContent({
  ...
  const paymentModeDetails = (mop, subMop) => {
    ...
    if (mop === "CARD") {
      subMopData =
        mopData?.list?.find((opt) => opt?.card_id === subMop) ||
        mopData?.stored_payment_details?.find((opt) => opt?.card_id === subMop);
    } else {
      subMopData = mopData?.list?.find((opt) => opt?.code === subMop);
    }
    return {
      mopData,
      subMopData,
  ...
  useEffect(() => {
    if (enableLinkPaymentOption) return;
  ...
  }, [paymentOption?.payment_option, enableLinkPaymentOption]);
