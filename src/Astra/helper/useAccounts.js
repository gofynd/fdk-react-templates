...
export const useAccounts = ({ fpi }) => {
	...
	const updateProfile = (data) => {
		...
		//     userFetched: rootState.auth.userFetched,
		//     userData: data,
		// };
		// commit(SET_USER_DATA, { data: users });
		// fireFPIEvent(FPI_EVENTS.PROFILE_UPDATE, {
		//     gender: data?.user?.gender,
		//     email: getPrimaryVerifiedValue(data?.user?.emails),
		//     phone_number: getPrimaryVerifiedValue(data?.user?.phone_numbers),
		//     user_id: data?.user?.user_id || data?.user?._id,
		// });
		// return data;
	};

	const signOut = () =>
		fpi.executeGraphQL(updateGraphQueryWithValue(LOGOUT), null).then((res) => {
			if (res?.user?.logout?.logout) {
				const queryParams = new URLSearchParams(location.search);
				const redirectUrl = queryParams.get("redirectUrl") || "";
				window.location.href = window.location.origin + decodeURIComponent(redirectUrl);
				return res;
