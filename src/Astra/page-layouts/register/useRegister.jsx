...
const useRegister = (fpi) => {
	...
	const isShowVerifyEmail = useMemo(() => !!verifyBothData?.verify_email_otp, [verifyBothData]);
	const isShowVerifyMobile = useMemo(() => !!verifyBothData?.verify_mobile_otp, [verifyBothData]);
	...
	const onRegisterFormSubmit = (formData) => {
		...
		signUp(user)
			...
			.catch((err) => {
				registerForm.setError("root", { message: err?.message || "Something went wrong" });
			});
	};
