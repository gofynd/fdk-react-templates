...
const useEditProfile = (fpi) => {
	...
	const userPrimaryEmail = useMemo(() => userData?.emails?.find((e) => e.primary), [userData]);
	...
	const handleCancelClick = (e) => {
		...
		localStorage.setItem("isCancelButtonClicked", EMAIL_MOBILE_SOFT_SHOW_PROFILE_TIME);
		openHomePage();
	};
	...
	const handleProfileUpdate = (formData) => {
		...
		updateProfile(user)
			...
			.catch((err) => {
				editProfileForm.setError("root", { message: err?.message || "Something went wrong" });
			});
	};
	return {
		editProfileForm,
